...
Pointt::Pointt(double _lon, double _lat, std::string _date) : lon(_lon), lat(_lat), date(_date) {
	Create_month_year();
}

void Pointt::Create_month_year() {
	if (date == "") {
		return;
	}
	year_record = std::stoi(date.substr(0, date.find("-")));
	//month_record = std::stoi(date.substr(date.find("-") + 1, date.find("-")));
	month_record = std::stoi(date.substr(date.find("-") + 1, 2));
}

void Pointt::PrintPoint() const {
